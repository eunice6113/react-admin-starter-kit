export interface Post {
    ...
    atchNo1:string;              //첨부파일번호
    rspbEmn: string;            //담당직원사번
    rspbEmm: string;            //담당직원이름
    sgcnCon: string;            //지원조건내용
    etcSvcCon: string;          //기타서비스
    srba:string;
    srwd:string;
    atchDetail?: Array<{ atchNo:string; atchSrn:string; atchNm:string; atchSizeVl:string;}>;
    atchDetail1?: Array<{ atchNo:string; atchSrn:string; atchNm:string; atchSizeVl:string;}>;
    fileTempCnt: number;
    fileTempCnt1: number;
}
